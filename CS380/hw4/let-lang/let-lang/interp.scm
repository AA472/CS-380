...
;;;;;;;;;;;;;;;; the interpreter ;;;;;;;;;;;;;;;;
  ...
  (define (value-of exp env)
      (cases expression exp
        ...
        [letrec-exp (func-names vars bodies let-body)
                    (value-of let-body
                              (extend-rec-env func-names vars bodies env))
                    ]
        ...
        [call-exp (operator operands)
                  (let* ([func-val (value-of operator env)]   ;; evaluate the operator
                         ...
                         [op-val (map (lambda(rand) (value-of rand env)) operands )])
...
(define (apply-proc func actual-arg)
  (cases proc func
    [procedure (formal body saved-env)
               (value-of body (extend-env* formal actual-arg saved-env))]
